...
public class AppTest {
    ...
    // BEGIN /people/{id} возвращает код ответа 200 и тип содержимого JSON.
    // Проверьте также, что вернувшийся JSON содержит нужные данные.
    @Test
    void specificUser() throws Exception {
        MockHttpServletResponse response = mockMvc
...
//PATCH-запрос на адрес /people/{id} возвращает код ответа 200 и обновляет данные персоны с указанным id.
    @Test
    void modifyUser() throws Exception {
        MockHttpServletResponse response = mockMvc
                .perform(patch("/people/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"firstName\": \"Aerton\", \"lastName\": \"Sena\"}")
                )
                .andReturn()
                .getResponse();
        assertThat(response.getStatus()).isEqualTo(200);
        ...
        assertThat(response1.getContentAsString()).contains("Aerton", "Sena");
    }
    @Test
    void deleteUser() throws Exception {
        MockHttpServletResponse response0 = mockMvc
                .perform(get("/people"))
                .andReturn()
                .getResponse();
        assertThat(response0.getContentAsString()).contains("John", "Smith");
