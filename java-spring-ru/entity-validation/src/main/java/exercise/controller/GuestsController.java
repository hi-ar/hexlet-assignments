...
public class GuestsController {
    ...
    public GuestDTO create(@Valid @RequestBody GuestCreateDTO dto) {
        System.out.println("±±±± dto is " + dto.getClubCard() + " / " + dto.getPhoneNumber());
        Guest g = guestMapper.map(dto);
        System.out.println("§§§ phone is " + g.getPhoneNumber());
        guestRepository.save(g);
        return guestMapper.map(g);
    }
    // END
}
