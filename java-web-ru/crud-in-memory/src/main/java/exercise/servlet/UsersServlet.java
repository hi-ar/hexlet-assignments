...
public class UsersServlet extends HttpServlet {
    ...
    private void createUser(HttpServletRequest request,
        ...
        Map<String, String> newUser = new HashMap<>(); //новый пользователь
        if (firstName.isEmpty() || lastName.isEmpty()) { //валидация: имя или фамил пустые
            String errorMsg = "Name and last name cant be empty";
        ...
        response.setStatus(200); //успех, переадресация
        request.setAttribute("users", users);
        RequestDispatcher requestDispatcher = request.getRequestDispatcher("/users.jsp");
        requestDispatcher.forward(request, response);
        // END
    ...
    private void editUser(HttpServletRequest request,
        ...
        request.setAttribute("error", ""); //возможно нужно отдельно поля отдавать
        request.setAttribute("user", user);
        requestDispatcher.forward(request, response);
        // END
    }
    ...
    private void updateUser(HttpServletRequest request,
        ...
        String email = request.getParameter("email");
        if (firstName.isEmpty() || lastName.isEmpty()) { //валидация: имя или фамил пустые
            String errorMsg = "Name and lastname cant be empty";
            response.setStatus(422);
            RequestDispatcher requestDispatcher = request.getRequestDispatcher("/edit.jsp");
            request.setAttribute("error", errorMsg);
            request.setAttribute("user", user);
        ...
        //users.add(user); // нужно ли добавлять пользователя в лист?
