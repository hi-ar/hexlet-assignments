...
public class App {
    ...
    public static void main(String[] args) throws LifecycleException, SQLException, IOException {
        // BEGIN созд соед с H2 (указ путь к файлу БД), выполните запросы из init.sql
        Connection connection = DriverManager.getConnection("jdbc:h2:./hexlet");
        Statement statement = connection.createStatement();
        String initSQL;
        try {
            initSQL = getFileContent("init.sql");
        } catch (RuntimeException e) {
            throw new RuntimeException("couldn't open init.sql");
        }
