...
public class Users {
    ...
    public Users() { //создает лист из 100 фейк-пользователей
    ...
    private String getNextId() { //айди след (нов) пользователя
        int id = ++idCounter;
    ...
    public Map<String, String> build(String firstName, String lastName, String email) {
        Map<String, String> user = new HashMap<>();
        // возвращает нов юзера без id, из фи+и, либо имейл, либо пустого
    ...
    public void add(Map<String, String> user) { //добавляет польз id и в лист
        user.put("id", getNextId());
        users.add(user);
    }

    public void update(Map<String, String> user, //заменяет юзера (стар, нов)
    ...
    public Map<String, String> findById(String id) { //нах по id
    ...
    public Map<String, String> findByEmail(String email) { //нах по email
        Map<String, String> user = users
            .stream()
